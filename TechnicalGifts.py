...
def create_output(tech_data: List) -> Dict[str, str]:
    output = {}
    areas = sorted(list(set([tech[1] for tech in tech_data])))

    for area in areas:
        area_index = areas.index(area)
        event_id = 10 + area_index
        area_output = ""
        ...
        for idx, tech in enumerate(techs_in_area):
            option_name = chr(ord("a") + idx)
            area_output += (
                f"\t# {tech[0]}\n"
                f"\toption = {{\n"
